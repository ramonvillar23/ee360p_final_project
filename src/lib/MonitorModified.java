...
public class MonitorModified {

	public MonitorModified(){
			...
			while(true){
				...
				if(readyToAwake){ //if wait has been called at least once
					for(Integer id : conditionVariables.keySet())
					{
						...
						{
							for(Integer i : conditionVariables.keySet())
							{
								if(conditionVariables.get(i) != null)
								{
									for(Condition c : conditionVariables.get(i))
									{
										c.signal();
									}
								}
